...
# FTP_PORTS:  Ports for FTP server EPSV mode
...
iptables -A FORWARD -p tcp -d $FTP --dport $FTP_PORTS -j ACCEPT

iptables -A FORWARD -p tcp -d $FTP_2 --dport 21 -j ACCEPT
iptables -A FORWARD -p tcp -d $FTP_2 --dport $FTP_PORTS -j ACCEPT
